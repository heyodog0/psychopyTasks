...
stim_duration = 0.5  # duration of each stimulus
isi_duration = 0.7  # inter-stimulus interval

clock = core.Clock()

# Define the sequence parameters
num_stimuli = 50  # Total number of stimuli
...
for stim_num, stim in enumerate(stimuli):
    ...
    if keys:
        for key, time in keys:
            if key == 'escape':  # Check for exit key
                
                # End of the experiment
                end_message = visual.TextStim(win, text='Experiment ended by user. Thank you!', color='white')
                end_message.draw()
                win.flip()
                core.wait(2.0)
            ...
            if key == 'space' and stim == target:
                print(f"Correct response to {stim} at {time:.3f} seconds")
            else:
                print(f"Incorrect response to {stim} at {time:.3f} seconds")
